...
/** Wall-clock time for the LilyPond (Save as Lilypond) export action to complete. */
export class SaveAsLilypondTime implements MetricPlugin {
  readonly name = 'saveAsLilypondTime';
  readonly meta = meta;
  ...
  async extractMetric(page: Page): Promise<MetricValue> {
    ...
    return { name: this.name, value: ps.saveAsLilypondTime, meta: this.meta };
  }
}