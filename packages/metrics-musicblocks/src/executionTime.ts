...
/** Wall-clock time from Play to run completion (interpreter throughput). */
export class ExecutionTime implements MetricPlugin {
  ...
  async extractMetric(page: Page): Promise<MetricValue> {
    ...
    return { name: this.name, value: ps.executionTime, meta: this.meta };
  }
}