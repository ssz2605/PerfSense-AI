...
/** Total time from cold start to the app being interactive (bootstrap scenario). */
export class BootstrapTotal implements MetricPlugin {
  readonly name = 'bootstrapTotal';
  readonly meta = meta;
  ...
  async extractMetric(page: Page): Promise<MetricValue> {
    ...
    return { name: this.name, value: ps.bootstrapTotal, meta: this.meta };
  }
}