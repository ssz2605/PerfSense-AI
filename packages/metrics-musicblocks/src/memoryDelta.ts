...
/** Heap growth between run start and run completion (leak signal). */
export class MemoryDelta implements MetricPlugin {
  readonly name = 'memoryDelta';
  readonly meta = meta;
  ...
  async extractMetric(page: Page): Promise<MetricValue> {
    ...
    return { name: this.name, value: ps.memoryDelta, meta: this.meta };
  }
}