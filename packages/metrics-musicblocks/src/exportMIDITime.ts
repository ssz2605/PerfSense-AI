...
/** Wall-clock time for the MIDI export action to complete. */
export class ExportMIDITime implements MetricPlugin {
  readonly name = 'exportMIDITime';
  readonly meta = meta;
  ...
  async extractMetric(page: Page): Promise<MetricValue> {
    ...
    return { name: this.name, value: ps.exportMIDITime, meta: this.meta };
  }
}